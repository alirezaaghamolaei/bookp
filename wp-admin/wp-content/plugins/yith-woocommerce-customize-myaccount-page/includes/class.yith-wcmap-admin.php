...
if ( ! class_exists( 'YITH_WCMAP_Admin' ) ) {
	...
	class YITH_WCMAP_Admin {
		...
		public function action_links( $links ) {
            ...
            $links[] = '<a href="' . YIT_Plugin_Licence()->get_license_activation_url() . '">' . __( 'License', 'yith-woocommerce-customize-myaccount-page' ) . '</a>';
			return $links;
		}
         ...
         * @return void
         */
		public function register_strings_translations(){
		    $items = yith_wcmap_get_endpoints();
            ...
            foreach( $items as $key => $options ) {
                ...
                if( isset( $options['children'] ) ) {
                    foreach ( $options['children'] as $child_endpoint => $child_options ) {
                        empty( $child_options['label'] ) || $this->register_single_string( $child_endpoint, $child_options['label'] );
                        empty( $child_options['url'] ) || $this->register_single_string( $child_endpoint . '_url', $child_options['url'] );
                    }
                }
            }
        }
