...
if ( ! class_exists( 'YITH_WCMAP_Frontend' ) ) {
	...
	class YITH_WCMAP_Frontend {
		...
		public function redirect_to_default(){
            ...
            if( ! get_option( 'yith_wcmap_is_my_account', true ) && ! isset( $_REQUEST['elementor-preview'] ) ) {
		...
		public function add_widget_custom_content( $data ){
		    ...
		    if( ! empty( $data ) ) {
                ...
                return $this->replace_recursive( $data );
            }
			...
			if( isset( $array['settings'] ) ) {
			    foreach( $array['settings'] as $key => &$data ){
                    ...
                    if ( isset( $shortcode_myaccount[0] ) ) {
                        ...
                        $data = str_replace( $default_myaccount_content, $new_myaccount_content, $data );
                    }
                }
			}
