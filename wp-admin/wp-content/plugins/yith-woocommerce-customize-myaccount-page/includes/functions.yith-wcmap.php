...
if( ! function_exists( 'yith_wcmap_get_endpoints' ) ) {
    ...
    function yith_wcmap_get_endpoints() {
        ...
        foreach ( $fields as $id => $field_option ) {
            ...
            ( empty( $options ) && isset( $all_default[ $id ] ) ) && $options = $all_default[ $id ];
            ...
            if( isset( $field_option['children'] ) ) {
                ...
                foreach ( $field_option['children'] as $child_id => $child ) {
                    ...
                    $options_default        = call_user_func( "yith_wcmap_get_default_{$child['type']}_options", $id );
                    // is empty check on default endpoint
                    ( empty( $child_options ) && isset( $all_default[ $id ] ) ) && $child_options = $all_default[ $id ];
                    // always merge with default
                    $children[ $child_id ]  = array_merge( $options_default, $child_options );
		...
		foreach ( $endpoints_keys as $endpoint ) {
			...
			switch( $endpoint ) {
				case 'orders':
					...
					$slug 		=  get_option( 'woocommerce_myaccount_orders_endpoint', 'orders' );
					break;
				case 'edit-account':
				...
				case 'downloads':
					...
					$slug 		=  get_option( 'woocommerce_myaccount_downloads_endpoint', 'downloads' );
					break;
				case 'dashboard':
...
if( ! function_exists( 'yith_wcmap_endpoints_option_default' ) ) {
    ...
    function yith_wcmap_endpoints_option_default(){
        ...
        foreach( $fields as $field ) {
            if( isset( $field['children'] ) ) {
                foreach ( $field['children'] as $child ) {
                    isset( $child['slug'] ) && $return[ $child['slug'] ] = $child['label'];
                }
                continue;
            }
            isset( $field['slug'] ) && $return[ $field['slug'] ] = $field['label'];
        }
...
add_action( 'yith_wcmap_print_single_link', 'yith_wcmap_print_single_link', 10, 2 );
...
if( ! function_exists('yith_wcmap_print_single_endpoint') ) {
    ...
    function yith_wcmap_print_single_endpoint( $endpoint, $options ) {
        ...
        if( $endpoint == 'orders' ) {
            ...
            $current == $view_order && $current = $options['slug'];
        }

        $classes = array();
        ! empty( $options['class'] )                                    && $classes[] = $options['class'];
        ( isset( $options['slug'] ) && $options['slug'] == $current )   && $classes[] = 'active';
...
if( ! function_exists('yith_wcmap_print_endpoints_group') ) {
    ...
    function yith_wcmap_print_endpoints_group( $endpoint, $options ) {
        ...
        foreach( $options['children'] as $child ) {
            if( isset( $child['slug'] ) && $child['slug'] == $current ) {
	            $options['open']    = true;
                $classes[]          = 'active';
                break;
            }
