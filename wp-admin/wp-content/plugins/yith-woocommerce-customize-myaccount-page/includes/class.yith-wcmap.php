...
if ( ! class_exists( 'YITH_WCMAP' ) ) {
	...
	class YITH_WCMAP {
		...
		private function _update_old_option(){
			...
			if( get_option('yith_wcmap_old_option_updated', 0 ) == 2 ) {
				return;
			}
			...
			if( ! $fields ) {
				// no update required
				update_option( 'yith_wcmap_old_option_updated', 2 );
				return;
			}
			...
			foreach( $fields as $field ) {
                ...
                $option = get_option( 'yith_wcmap_endpoint_' . $field['id'], array() );

                $id = $this->_update_old_id( $field['id'], $option );

			    $updated[ $id ] = array();
                $updated[ $id ][ 'type' ] = isset( $option['url'] ) ? 'link' : 'endpoint';

                if( isset( $field['children'] ) ) {
                    $updated[ $id ][ 'type' ]      = 'group';
                    $updated[ $id ][ 'children' ]  = array();
                    foreach( $field['children'] as $child ){
                        $option     = get_option( 'yith_wcmap_endpoint_' . $child['id'], array() );
                        $child_id   = $this->_update_old_id( $child['id'], $option );

                        $updated[ $id ][ 'children' ][ $child_id ]              = array();
                        $updated[ $id ][ 'children' ][ $child_id ][ 'type' ]    = isset( $option['url'] ) ? 'link' : 'endpoint';
                    }
                }
			}

			update_option( 'yith_wcmap_endpoint', json_encode( $updated ) );
			update_option( 'yith_wcmap_endpoint_old', json_encode( $fields ) );
			update_option( 'yith_wcmap_old_option_updated', 2 );
		}

        /**
         * Update old option ID
         *
         * @param string $id
         * @param array $option
         * @return string
         */
		private function _update_old_id( $id, $option ) {
            if( $id == 'view-order' || $id == 'my-downloads' ){
                $id = ( $id == 'view-order' ) ? 'orders' : 'downloads';
                update_option( 'yith_wcmap_endpoint_' . $id , $option );
            }
